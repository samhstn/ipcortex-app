...
function request (method, url, options) {
  return new Promise(function (resolve, reject) {
    ...
    xhr.open(method, url);
    options ? xhr.send(JSON.stringify(options.payload)) : xhr.send();
  });
}

request('get', 'https://localhost:8000/extusers/credentials')
  .then((data) => {
    const username = data.data.uname;
    console.log(data.data.user);
    return IPCortex.PBX.Auth.login({ username: username, checkonly: true });
  })
  .then(function (data) {
    console.log(data, '<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<');
    ...
    const options = {
      payload: {
        cID: IPCortex.PBX.Auth.id
      }
    };
    console.log(IPCortex.PBX.Auth.id, '<<<<<<<id');
    return request('post', 'https://localhost:8000/extusers/room', options);
  })
  .then(function (data) {
    console.log('Login responded with:', data);
  })
  .catch(function () {
    console.log('Failed');
  });
